...
export default function ProductPage({ params }: { params: { id: string } }) {
    const productMeta: Record<string, { designer: string }> = {
        'a1': { designer: 'Designer A' },
        'a2': { designer: 'Designer A' },
        'b1': { designer: 'Designer B' },
    }
    const designer = productMeta[params.id]?.designer ?? 'Unknown Designer'
    ...
    return (
        ...
        <div className="relative z-10 w-full max-w-4xl bg-white rounded-2xl shadow-xl max-h-[85vh] flex flex-col">
            ...
            <button
                onClick={() =>
                router.push(`/collections?designer=${encodeURIComponent(designer)}`)
                }
                className="
